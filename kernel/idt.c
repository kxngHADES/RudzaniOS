...
/* IDT entry structure (8 bytes each, must be packed) */
struct idt_entry {
	uint16_t base_low;
	uint16_t sel;
	uint8_t always0;
...
/* IRQ handler table (16 hardware IRQs) */
static irq_handler_func_t irq_handlers[16] = { 0 };
...
/* External IRQ stubs defined in isr.asm */
...
static void pic_remap(void) {
	...
	/* ICW2: Vector offsets (remap IRQs away from CPU exceptions) */
	port_byte_out(0x21, 0x20);  /* Master PIC: IRQ 0-7 -> INT 32-39 */
	port_byte_out(0xA1, 0x28);  /* Slave PIC:  IRQ 8-15 -> INT 40-47 */
	...
	/* Clear all IRQ masks (enable all) */
...
/* Called from isr.asm for CPU exceptions (INT 0-31) */
void isr_handler(registers_t *regs) {
	if (regs->int_no < 32) {
		...
		/* Halt on unrecoverable exception */
...
/* Called from isr.asm for hardware IRQs (INT 32-47) */
void irq_handler(registers_t *regs) {
	int irq_num = regs->int_no - 32;

	/* Call the installed handler if any */
